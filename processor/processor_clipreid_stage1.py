...
def do_train_stage1(cfg,
    ...
    with torch.no_grad():
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader_stage1):
            ...
            with amp.autocast(enabled=True):#混合精度使用，在效果相同的情况下使用低精度代替高精度输入，降低计算开销
                image_feature = model(img, target, get_image = True)#获得图像的映射
        ...
        num_image = labels_list.shape[0]#获取图像总数
        i_ter = num_image // batch#计算训练图像需要几个批次
    del labels, image_features

    for epoch in range(1, epochs + 1):
        loss_meter.reset()
        scheduler.step(epoch)#传入epoch参数可以不按照原来优化器中的学习率学习，即使用wormlabel
        model.train()

        iter_list = torch.randperm(num_image).to(device)#生成0到num_image-1的随机列表
